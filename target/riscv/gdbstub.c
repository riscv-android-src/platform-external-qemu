...
static int riscv_gdb_get_fpu(CPURISCVState *env, uint8_t *mem_buf, int n)
{
    ...
    /* there is hole between ft11 and fflags in fpu.xml */
        ...
        //result = riscv_csrrw(env, n - 33 +  8, &val, 0, 0);
        //if (result == 0) {
...
static int riscv_gdb_set_fpu(CPURISCVState *env, uint8_t *mem_buf, int n)
{
    ...
    /* there is hole between ft11 and fflags in fpu.xml */
        ...
        //result = riscv_csrrw(env, n - 33 + 8, NULL, val, -1);
        //if (result == 0) {
        //    return sizeof(target_ulong);
        //}
    }
...
//static int riscv_gdb_get_vext(CPURISCVState *env, uint8_t *mem_buf, int n)
//{
//    int i = 0, tot = 0;
//    if (n >= 0 && n < 32) {
//        for (; i < VLEN / 64; i++) {
//            tot += gdb_get_reg64(mem_buf + 8 * i,
//                    env->vfp.vreg[n].u64[i]);
//        }
//    }
//    return tot;
//}
//
//static int riscv_gdb_set_vext(CPURISCVState *env, uint8_t *mem_buf, int n)
//{
//    int i = 0;
//    if (n >= 0 && n < 32) {
//        for (; i < VLEN / 64; i++) {
//            env->vfp.vreg[n].u64[i] = ldq_p(mem_buf + 8 * i);
//        }
//        return VLEN / 8;
//    }
//    return 0;
...
void riscv_cpu_register_gdb_regs_for_features(CPUState *cs)
{
    ...
    gdb_register_coprocessor(cs, riscv_gdb_get_csr, riscv_gdb_set_csr,
                             245, "riscv-32bit-csr.xml", 0);
#elif defined(TARGET_RISCV64)
    gdb_register_coprocessor(cs, riscv_gdb_get_csr, riscv_gdb_set_csr,
                             245, "riscv-64bit-csr.xml", 0);
#endif
    //if (env->misa & RVV) {
    //    gdb_register_coprocessor(cs, riscv_gdb_get_vext, riscv_gdb_set_vext,
    //                             32, "riscv-64bit-vext.xml", 0);
    //}
}
